...
get_dbus_env() {
    local pid
    pid=$(su -c "chroot '$DEBIANPATH' /usr/bin/pgrep -x -n xfsettingsd 2>/dev/null || chroot '$DEBIANPATH' /usr/bin/pgrep -x -n xfwm4 2>/dev/null || chroot '$DEBIANPATH' /usr/bin/pgrep -x -n xfce4-panel 2>/dev/null" 2>/dev/null)
    if [ -n "$pid" ]; then
        local bus
...
case "${1:-status}" in
    ...
    status|"")
        if su -c "grep -q -w '$DEBIANPATH/proc' /proc/mounts" 2>/dev/null; then
            ...
            current=""
            if [ -n "$dbus_env" ]; then
                current=$(su -c "chroot '$DEBIANPATH' /bin/bash -c 'export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export DISPLAY=:0; $dbus_env timeout -k 1s 2 xfconf-query -c xsettings -p /Net/ThemeName 2>/dev/null'" 2>/dev/null)
            fi
            if [ -z "$current" ]; then
                current=$(su -c "sed -n -E 's/.*<property name=\"ThemeName\" type=\"string\" value=\"([^\"]*)\".*/\1/p' '$DEBIANPATH/root/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml'" 2>/dev/null | head -n1)
            fi
