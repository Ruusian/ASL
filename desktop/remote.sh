...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    ...
    case "$TARGET" in
        ...
        help|-h|--help)
            ...
            echo "  oracle [start|stop]  Control dedicated Oracle Cloud VPS tunnel"
            echo "  oracle config [args] Configure VPS host, user, port"
            echo "  oracle add-key <key> Import private key for Oracle VPS"
            echo "  oracle gen-key       Generate new dedicated SSH keypair"
            echo "  oracle push-pubkey   Push public key to Oracle VPS authorized_keys"
            echo "  oracle remove        Completely remove Oracle VPS configuration and keys"
