...
if [ "$mounted" = 1 ]; then
    (asl_chroot_exec "test -x /usr/bin/xfwm4 -o -x /usr/bin/xfce4-session" 2>/dev/null) && check xfce optional "XFCE session / window manager available" PASS || check xfce optional "install with: asl desktop setup" WARN
    (asl_chroot_exec "test -x /usr/bin/dbus-launch -o -x /usr/bin/dbus-daemon" 2>/dev/null) && check dbus optional "D-Bus daemon & launcher ready" PASS || check dbus optional "install with: asl desktop setup" WARN
    (asl_chroot_exec "test -s /etc/machine-id" 2>/dev/null) && check machine-id optional "system machine-id initialized" PASS || check machine-id optional "/etc/machine-id uninitialized (run: asl repair)" WARN
    if asl_chroot_exec "find /usr/share/vulkan/icd.d -type f -name '*.json' -print -quit 2>/dev/null | grep -q ." 2>/dev/null; then check vulkan optional "ICD JSON found" PASS; else check vulkan optional "no Vulkan ICD JSON found (run: asl gpu apply)" WARN; fi
fi
exit "$fail"
