...
asl_repair_run() {
    ...
    if [ "$target" = "dpkg" ] || [ "$target" = "all" ]; then
        echo "[3/4] Repairing Debian DPKG / APT lock states..."
...
case "${1:-all}" in
    run|all|mounts|permissions|dpkg|env)
        asl_repair_run "${1:-all}"
        ;;
    *)
        echo "Usage: asl repair [all|mounts|permissions|dpkg|env]"
        ;;
esac
