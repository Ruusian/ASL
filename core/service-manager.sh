...
# Locate ASL directory and start 24/7 background services
ASL_PATH="$HOME/ASL"
if [ -f "$ASL_PATH/bin/asl" ]; then
    bash "$ASL_PATH/bin/asl" service start >> "$PREFIX/tmp/asl-boot.log" 2>&1
fi
BOOT_EOF
...
asl_service_status() {
    ...
    ssh_pid=$(pgrep -f "sshd" 2>/dev/null | head -n 1 || true)
    if [ -n "$ssh_pid" ]; then
